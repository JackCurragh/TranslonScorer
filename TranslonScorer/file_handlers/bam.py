...
def getexons_and_cds(annotation_file, tran=[]):
    ...
    cds_df = cds_df.group_by("tran_id").agg(
        [
            pl.col("chr").first(),
    ...
    exon_df = exon_df.group_by("tran_id").agg(
        [
            pl.col("chr").first(),
...
def get_bam_tran(bam_df, exon_df):
    ...
    for start in range(chr_min, chr_max, window_size):
        ...
        window_reads = bam_df.filter((pl.col("start") >= start) & (pl.col("start") < end))
        if window_reads.is_empty():
            continue

        window_exons = exon_df.filter((pl.col("start") < end) & (pl.col("stop") > start))
        if window_exons.is_empty():
            continue

        # Find overlaps within the window using a more efficient join
        overlaps = window_reads.join(window_exons, on="chr", how="inner").filter(
            (pl.col("start") >= pl.col("start_right")) & (pl.col("stop") <= pl.col("stop_right"))
        )
        ...
        if not overlaps.is_empty():
            ...
            ).select(pl.all().exclude("start_right", "stop_right", "tran_start", "tran_stop"))
    ...
    return pl.concat(results).unique(subset=dedupe_subset, maintain_order=True)
...
def bamtranscript(bam_df, exon_df):
    ...
    log_info(
        f"Found {len(uniquechr_bam)} unique chromosomes in BAM and {len(uniquechr_exon)} in annotation"
    )
            ...
            if exon_has_chr:
                ...
                bam_df = bam_df.with_columns(
                    [
                        ...
                        .then(pl.col("chr"))
                        .otherwise(
                            pl.concat_list([pl.Series("chr", ["chr"]), pl.col("chr")]).list.join("")
                        )
                        .cast(pl.Categorical)
                        .alias("chr")
                    ]
    ...
    for idx, chr in enumerate(sorted(common_chr), 1):
        ...
        exon_with_chr = exon_df.filter(pl.col("chr") == chr).explode(
            ["start", "stop", "tran_start", "tran_stop"]
        )
...
def process_transcriptomic_bam(df_namesplit, cds_df):
    ...
    bam_to_cds = bam_to_cds.join(
        cds_df.select(["tran_id", "start"]).rename({"start": "cds_start"}), on="tran_id", how="left"
    )

    # Calculate position relative to CDS start
    bam_to_cds = bam_to_cds.with_columns(bamcds_start=pl.col("start") - pl.col("cds_start")).drop(
        "cds_start"
    )
