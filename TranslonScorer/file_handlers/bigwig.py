    ...
    try:
        ...
        if isinstance(tran_data, dict) and "positions" in tran_data:
            ...
            tran_reads = pl.DataFrame(
                {"tran_start": pl.arange(0, max_pos, eager=True), "counts": full_array}
            )
        elif isinstance(tran_data, list):
            # Convert from simple list
            tran_reads = pl.DataFrame(
                {"tran_start": pl.arange(0, len(tran_data), eager=True), "counts": tran_data}
            )
        else:
            # Invalid data format
            return None
        ...
        for typeorf in orfs["type"].unique():
            ...
            try:
                ...
                log_info(
                    f"Time taken to score ORF type {typeorf} for {tran_id}: {scoring_duration:.4f} seconds"
                )
            except Exception as e:
...
def scoring(
    ...
    frame_support_path: Optional[str] = None,
):
    """
    ...
    if isinstance(exon, str):
        if os.path.exists(exon) and os.path.getsize(exon) > 1e9:  # 1 GB
            ...
            exon_df = (
                pl.scan_csv(exon)
                .select(["chr", "start", "stop", "tran_id", "tran_start", "tran_stop"])
                .collect()
            )
        else:
            ...
            for col in ["start", "stop", "tran_start", "tran_stop"]:
                if col in exon_df.columns:
                    exon_df = exon_df.with_columns(
                        pl.col(col)
                        .map_elements(lambda x: x.split(",") if isinstance(x, str) else x)
                        .alias(col)
                    )
    else:
        exon_df = exon
    ...
    if stranded:
        ...
        log_info(
            f"Found {len(pos_strand_orfs)} positive strand, {len(neg_strand_orfs)} negative strand, and {len(unstrand_orfs)} unstranded ORFs"
        )
        ...
        if not pos_strand_orfs.is_empty():
            ...
            pos_results = process_strand_orfs(
                forward_bw,
                exon_df,
                pos_strand_orfs,
                old_scoring,
        ...
        if not neg_strand_orfs.is_empty():
            ...
            neg_results = process_strand_orfs(
                reverse_bw,
                exon_df,
                neg_strand_orfs,
                old_scoring,
        ...
        if not unstrand_orfs.is_empty():
            ...
            unstrand_results = process_strand_orfs(
                forward_bw,
                exon_df,
                unstrand_orfs,
                old_scoring,
...
def process_strand_orfs(
    ...
    transcript_batches = [
        list(transcript_ids)[i : i + batch_size] for i in range(0, len(transcript_ids), batch_size)
    ]
    ...
    for batch_idx, transcript_batch in enumerate(transcript_batches):
        log_info(
            f"Processing batch {batch_idx+1}/{len(transcript_batches)} ({len(transcript_batch)} transcripts)"
        )
...
def score_single_transcript(
    ...
    try:
                ...
                try:
                    ...
                    if not fs_tx.is_empty():
                        ...
                        coverage_df = coverage_df.with_columns(
                            (pl.col("tran_start") // 3).alias("codon_idx")
                        )
                        coverage_df = coverage_df.join(fs_tx, on="codon_idx", how="left").fill_null(
                            1.0
                        )
                        coverage_df = coverage_df.with_columns(
                            (pl.col("counts") * pl.col("p0")).alias("counts")
                        ).select(
                            ["tran_start", "counts"]
                        )  # keep schema
                except Exception as e:
            ...
            for orf_type in orf_df["type"].unique():
                ...
                if old_scoring:
                    ...
                    if not empty_score_df.is_empty():
                        # Calculate new scores
                        score_dict = newscoring(
                            empty_score_df, coverage_df, sru_range, orf_type, score_dict
                        )
        ...
        log_error(
            f"Error in score_single_transcript for {tran_id}: {str(e)}", raise_exception=False
        )
        return None
...
def add_strand_to_orfs(orf_df, exon_df):
    ...
    orf_df = orf_df.with_columns(
        pl.col("tran_id")
        .map_elements(
            lambda tran_id: tran_strands.get(tran_id, "+")  # Default to + if transcript not found
        )
        .alias("strand")
    )
