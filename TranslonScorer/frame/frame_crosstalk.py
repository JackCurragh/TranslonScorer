...
def estimate_crosstalk_matrix(
    profiles: pl.DataFrame, cds_windows: pl.DataFrame, length: int
    ...
    joined = (
        p.join(cds_windows, on="tran_id", how="inner")
    ...
    sums = np.array(
        [float(agg["sum"][i]) if i < agg.height else 0.0 for i in range(3)], dtype=float
    )
