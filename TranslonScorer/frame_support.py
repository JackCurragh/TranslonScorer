"""Codon-level frame posteriors from P/A-site profiles (pure transforms).

Consumes already-built P/A-site profiles — does NOT infer offsets or build
profiles.  The caller must supply profiles with correct site coordinates before
...
def build_frame_support(
    ...
    cds_df: pl.DataFrame,
    params: FrameSupportParams = FrameSupportParams(),
) -> pl.DataFrame:
    """Build codon-level frame posteriors from already-built P/A-site profiles.

    Parameters
    ----------
    profiles : tidy profiles with columns tran_id, pos, count[, length].
               Must NOT require offset inference — offsets must already be applied.
    cds_df   : CDS annotation used to learn the confusion matrix (training signal).
    params   : FrameSupportParams controlling the method choice.

    Returns
    -------
    DataFrame: tran_id, codon, p0, p1, p2, method[, length, entropy, qc_flags, …]
    """
    method = (params.frame_method or "none").lower()
    if method == "none":
        log_info("Frame method=none; skipping frame support")
        return _empty_frame_support(has_len=False)

    has_len = ("length" in profiles.columns) and bool(params.frame_by_length)
    ...
    if method == "latent":
        out = _build_latent(prof_use, cds_df, has_len, method, params.frame_background)
    else:
        out = _build_linear(prof_use, cds_df, has_len, method)

    if method.endswith("+hmm") and not out.is_empty():
        out = _hmm_smooth(out, has_len, params.frame_hmm_lambda)

    return _add_support_diagnostics(out)
