...
def cigar_blocks(
    ref_start: int, cigar_tuples: List[Tuple[int, int]]
) -> List[Tuple[int, int]]:
...
def aggregate_junctions(bam_path: str) -> pl.DataFrame:
    ...
    if not counts:
        return pl.DataFrame(schema={
            "chr": pl.Utf8, "donor_pos": pl.Int64,
            "acceptor_pos": pl.Int64, "strand": pl.Utf8, "count": pl.Int64,
        })
    rows = [{"chr": c, "donor_pos": d, "acceptor_pos": a, "strand": s, "count": cnt}
...
def readbam(
    ...
    *,
    ...
    if ox is not None:
        ...
        if not rows:
            return pl.DataFrame(schema={
                "chr": pl.Utf8, "start": pl.Int64, "stop": pl.Int64,
                "length": pl.Int64, "strand": pl.Utf8, "count": pl.Int64,
            })
    ...
    if "flag" in df.columns:
        df = df.with_columns(
            pl.when(pl.col("flag") & 0x10 > 0).then(pl.lit("-")).otherwise(pl.lit("+")).alias("strand")
        ).drop("flag")
    ...
    if collapsed and count_from == "name":
        ...
        if count_tag:
            try:
                ...
                if "qname" in df.columns:
                    df = df.with_columns(
                        pl.col("qname").map_elements(lambda s: q2c.get(s, 1), return_dtype=pl.Int64).alias("count")
                    )
                else:
