...
def assemble_translome(orfs_parquet: str, out_parquet: str, solver: str = 'PULP', timeout_sec: int = 60) -> str:
    ...
    if solver.upper() == 'PULP' and HAVE_PULP:
        ...
        xs = {i: pulp.LpVariable(f"x_{i}", lowBound=0, upBound=1, cat='Binary') for i in range(orfs.height)}
