...
def _scan_gtf_exons(
    gtf_path: str, chrom_filter: Optional[Set[str]] = None
...
def _build_tx_models_streaming(gtf_path: str, chroms: Set[str]) -> Dict[str, TxModel]:
    ...
    for tx, exons in exons_by_tx.items():
        ...
        for s, e in exons:
            cum.append(acc)
            acc += e - s
        tx_models[tx] = TxModel(
            chrom, strand, exons, cum, acc, str(gene_id) if gene_id is not None else None
        )
    return tx_models
