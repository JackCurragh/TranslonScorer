...
def _make_transcripts_table(exon_df: pl.DataFrame, fmap_df: Optional[pl.DataFrame]) -> pl.DataFrame:
    ...
    lens = (
        exon_df
        .with_columns(
            pl.struct(['tran_start', 'tran_stop']).map_elements(
...
def build_annotation_bundle(gtf_path: str, out_dir: str, *, progress: bool = True) -> Tuple[str, Dict[str, str]]:
...
def load_annotation_bundle(dir_path: str) -> Tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame, pl.DataFrame, pl.DataFrame, Optional[str], Dict[str, Any]]:
