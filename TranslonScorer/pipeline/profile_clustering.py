...
def _kmeans_cosine(
    ...
    *,
    ...
    Returns integer label array of shape (n_samples,).
    """
    rng = np.random.default_rng(random_state)
    X_n = _l2_normalise(X)   # cosine distance ≡ Euclidean on L2-normalised
    ...
    for _ in range(n_init):
        ...
        for _ in range(max_iter):
            # Assign: cosine similarity = dot product after L2 normalisation
            sims = X_n @ centers.T                    # [n, k]
...
def _agglomerative_cosine(X: np.ndarray, k: int) -> np.ndarray:
    """Agglomerative clustering with cosine distance (requires scipy)."""
    try:
    ...
    return fcluster(Z, k, criterion="maxclust") - 1   # 0-based
...
def _agglomerative_distance(
    ...
    *,
    ...
    """Agglomerative clustering with scipy, cut by threshold or fixed k."""
    try:
        from scipy.cluster.hierarchy import linkage, fcluster
        from scipy.spatial.distance import pdist
    except ImportError as exc:
...
def cluster_locus_profiles(
    ...
    *,
    ...
    if X.shape[0] != len(sample_names):
        raise ValueError(
            f"matrix has {X.shape[0]} rows but {len(sample_names)} sample names"
        )
...
def cluster_locus_shape_homogeneous(
    ...
    *,
    ...
    """Cluster samples into homogeneous locus-profile shapes.

    This method is deliberately stricter and simpler than general-purpose
    clustering.  Each sample is converted to a probability profile over the
    locus, optionally smoothed, and clustered by a distributional distance.
    Clusters that are not internally homogeneous are pruned, and low-coverage
    samples are never allowed to define clusters.
    """
    ...
    if len(included_idx) > 0:
        ...
        if centroids:
            ...
            C = np.vstack([centroids[cid] for cid in active_centroids])
...
def explain_shape_cluster_drivers(
    ...
    *,
    ...
    The diagnostics use the same probability-profile geometry as
    ``cluster_locus_shape_homogeneous``.  For each accepted cluster pair, the
    returned contrast table ranks positions by their contribution to the
    distance between the two cluster centroids.  This is intentionally
    position-level: callers can aggregate neighbouring rows into broader
    windows for plotting/reporting.
    """
    X = np.asarray(raw_matrix, dtype=float)
    if X.ndim != 2:
        raise ValueError(f"Expected a 2D matrix, got shape {X.shape}")
    ...
    if positions is None:
        ...
        if pos.shape[0] != X.shape[1]:
            raise ValueError(
                "positions must contain one value per matrix column "
    ...
    labels = np.array([label_lookup.get(str(sample), -1) for sample in sample_names], dtype=int)
...
def aggregate_score_profiles_by_cluster(
    ...
    *,
    ...
    """Aggregate accepted clusters from ``X_score``; ``X_cluster`` is never used."""
...
def cluster_profiles(
    ...
    *,
    ...
    Parameters
    ----------
    matrix        [n_samples × n_positions] float array (should be normalised).
    sample_names  list of length n_samples.
    n_clusters    desired number of clusters.
    method        'kmeans' (default, no extra deps) or 'agglomerative' (scipy).
    min_coverage  samples whose row-sum < min_coverage are excluded (label = -1).

    Returns
    -------
    labels            int array shape (n_samples,); -1 for excluded samples.
    included_samples  names of samples that were clustered.
    excluded_samples  names of samples below min_coverage.
    """
    if len(sample_names) != matrix.shape[0]:
        raise ValueError(
            f"matrix has {matrix.shape[0]} rows but {len(sample_names)} sample names"
        )
...
def aggregate_cluster_profiles(
    ...
    *,
    ...
    """Compute one aggregate profile per cluster.

    Parameters
    ----------
    matrix   [n_samples × n_positions].
    labels   int array (n_samples,); -1 entries are ignored.
    method   'mean' (default) or 'sum'.

    Returns  {cluster_id: profile_array}
    """
...
# ---------------------------------------------------------------------------
# Helpers for building profile matrices from a profiles DataFrame
# ---------------------------------------------------------------------------
...
def build_profile_matrix(
    ...
    *,
    ...
    profiles must have columns: sample_id, pos, count.

    Returns
    -------
    matrix    float32 [n_samples × n_positions]
    pos_vec   int64  [n_positions] unique sorted positions
    """
