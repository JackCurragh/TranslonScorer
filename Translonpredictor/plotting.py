...
def pertranscriptplot(df, exon_df, bwfile):
    ...
    table = go.Figure(data=[go.Table(
        header=dict(values=list(df_type_filtered.columns),
                fill_color='paleturquoise',
                align='left'),
        cells=dict(values=[df_type_filtered.tran_id, df_type_filtered.start, df_type_filtered.stop,
                           df_type_filtered.length,df_type_filtered.startorf,df_type_filtered.stoporf,df_type_filtered.type,
                           df_type_filtered.rise_up,df_type_filtered.step_down,df_type_filtered.hrf,df_type_filtered.avg,
                           df_type_filtered.nzc,df_type_filtered.score, df_type_filtered.frame],
               fill_color='lavender',
               align='left'))
        ])
    ...
    for typeorf in df['type'].unique():
        ...
        for tran in df_type_filtered['tran_id'].unique():
            ...
            if not tran_reads.is_empty():
                ...
                startplot = tran_reads.group_by("tran_start")\
                                       .agg(pl.col("counts").sum())
                for start in starts:
                    startplot_final = startplot.with_columns(
                ...
                stopplot = tran_reads.group_by("tran_stop")\
                                       .agg(pl.col("counts").sum())
                for stop in stops:
                    stopplot_final = stopplot.with_columns(
        ...
        start_dict = {'relativeloc':list(metagene_start_dict.keys()), 'counts':list(metagene_start_dict.values())}
        stop_dict = {'relativeloc':list(metagene_stop_dict.keys()), 'counts':list(metagene_stop_dict.values())}
        #plot metagene per type
        fig_combined_stop = px.bar(
        start_dict, x="relativeloc", y="counts"
        )
        ...
        fig_combined = px.bar(
        stop_dict, x="relativeloc", y="counts", title=typeorf,
        )
